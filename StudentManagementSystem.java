...
public class StudentManagementSystem {
    ...
    // SHA-256 Encrypted Hash of "password123" for maximum source-code security
    static final String ADMIN_USERNAME = "admin";
    static final String ADMIN_PASSWORD_HASH = "ef92b778bafe771e89245b89ecbc08a44a4e166c06659911881f383d4473e94f";
    ...
    // ======================================
    // Smart Input Validations (Using Regex)
    // ======================================
    static String getValidMobile() {
        while (true) {
            System.out.print("Mobile No (10 Digits): ");
            String mobile = sc.nextLine().trim();
            if (Pattern.matches("^[0-9]{10}$", mobile)) return mobile;
    ...
    static String getValidEmail() {
        while (true) {
            ...
            if (Pattern.matches("^[a-zA-Z0-9_+&*-]+(?:\\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,7}$", email)) return email;
            System.out.println("[✗] Error: Invalid email format. Please enter a valid address (e.g., alex@domain.com).");
        }
    }
    ...
    static String getValidDOB() {
        while (true) {
            ...
            if (Pattern.matches("^\\d{2}-\\d{2}-\\d{4}$", dob)) return dob;
            System.out.println("[✗] Error: Invalid date format. Please use the DD-MM-YYYY format.");
        }
    }
    ...
    static boolean adminLogin() {
        ...
        if (username.equals(ADMIN_USERNAME) && hashPassword(password).equals(ADMIN_PASSWORD_HASH)) {
            System.out.println("\n[✓] Login Successful! Access granted to the system.");
            return true;
        } else {
    ...
    static int getNextRollNumber() {
        if (students.isEmpty()) return 101; // Begins at 101 for the first student
        int maxRoll = 0;
        for (Student st : students) {
            if (st.rollNo > maxRoll) maxRoll = st.rollNo;
        }
        return maxRoll + 1; // Increment by 1 automatically
    }
    ...
    static void saveToFile() {
        try {
            ...
            for(Student st : students) {
                ...
                sb.append(st.rollNo).append(",");
                sb.append(st.name).append(",");
                sb.append(st.dob).append(",");
                sb.append(st.fatherName).append(",");
                sb.append(st.course).append(",");
                sb.append(st.branch).append(",");
                sb.append(st.year).append(",");
                sb.append(st.mobileNo).append(",");
                sb.append(st.email).append(",");
                sb.append(st.address).append(",");
                sb.append(st.semester).append(",");

                for(int i = 0; i < 5; i++) {
                    sb.append(st.subjects[i] == null ? "NA" : st.subjects[i]).append(",");
                }
                for(int i = 0; i < 5; i++) {
                    sb.append(st.marks[i]);
                    if(i < 4) sb.append(",");
                }
                sb.append("\n");
    ...
    static void smartSearch() {
        ...
        System.out.println("2. Search by Student Name (Partial/Full)");
        ...
        if (ch == 1) {
            ...
            System.out.print("Enter Branch Name (e.g., CSE, ME): ");
            String branch = sc.nextLine().toLowerCase();
            boolean found = false;
            for (Student st : students) {
                if (st.branch.toLowerCase().equalsIgnoreCase(branch)) {
                    printStudentDetails(st);
                    found = true;
                }
            }
    ...
    // ======================================
    // Export Data to Excel/CSV Report
    // ======================================
    static void exportToCSV() {
        ...
        try (PrintWriter pw = new PrintWriter(new File(CSV_FILE_NAME))) {
            pw.println("Roll No,Name,DOB,Father Name,Course,Branch,Year,Mobile,Email,Address,Semester,Total Marks,Percentage");
            for (Student st : students) {
                int total = 0;
                for (int m : st.marks) total += m;
                double perc = total / 5.0;

                pw.printf("%d,%s,%s,%s,%s,%s,%d,%s,%s,%s,%d,%d,%.2f%%\n",
                    st.rollNo, st.name, st.dob, st.fatherName, st.course, st.branch, 
                    st.year, st.mobileNo, st.email, st.address, st.semester, total, perc);
            }
            System.out.println("[✓] Report successfully exported to " + CSV_FILE_NAME + " (Compatible with Microsoft Excel)");
        } catch (Exception e) {
            System.out.println("[✗] Critical failure during CSV spreadsheet conversion.");
        }
    }

    // ======================================
    // Dynamic Performance Analytics Dashboard
    // ======================================
    static void viewAnalytics() {
        ...
        for (Student st : students) {
            ...
            for (int m : st.marks) {
                ...
                if (m < 40) failedInAny = true; // Critical performance boundary threshold (40)
            }
        ...
        System.out.println("Batch Average Score     : " + String.format("%.2f", (batchTotalPercentage / totalStudents)) + "%");
        System.out.println("College Topper          : " + topper.name + " (Roll No: " + topper.rollNo + ") with " + (topperTotal/5.0) + "%");
        System.out.println("Students Needing Focus  : " + failedCount + " (Students failed / Grade F)");
        System.out.println("============================================");
    }
    ...
    static void printStudentDetails(Student st) {
        System.out.println("\n----------------------------------");
        System.out.println("Roll No       : " + st.rollNo + " [AUTO]");
    ...
    // ======================================
    // Student Registration (With Validation & Auto Roll No)
    // ======================================
    static void registerStudent() {
        System.out.println("\n===== STUDENT REGISTRATION =====");
        int rollNo = getNextRollNumber();
        System.out.println("Assigned Roll No : " + rollNo + " (Generated Automatically)");
    ...
    static void enterMarks() {
        ...
        if(st.subjects[0] == null || st.subjects[0].equals("NA")) {
            System.out.println("[✗] Error: Please submit the Exam Form (Option 6) for this student first.");
            return;
        }
    ...
    public static void main(String[] args) {
        ...
        if (!adminLogin()) {
            System.out.println("Unauthorized configuration environment. Closing system.");
            return; 
        }
        ...
        do {
            ...
            System.out.println("3. Smart Search & Filters (Name/Branch/Roll)");
            ...
            System.out.println("10. View Batch Analytics Dashboard (Topper/Avg)");
            ...
            switch(choice) {
                ...
                default: System.out.println("[✗] Invalid Choice! Please choose between option numbers 1 to 12.");
            }
        } while(choice != 12);
    }
}